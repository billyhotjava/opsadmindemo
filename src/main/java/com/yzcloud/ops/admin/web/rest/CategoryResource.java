...
/**
 * REST controller for managing {@link com.yzcloud.ops.admin.domain.Category}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class CategoryResource {
    ...
    public ResponseEntity<Category> partialUpdateCategory(
        ...
        Optional<Category> result = categoryRepository
            ...
            .map(existingCategory -> {
                ...
                if (category.getInfo() != null) {
                    existingCategory.setInfo(category.getInfo());
                }
                ...
                if (category.getModifiedTime() != null) {
                    existingCategory.setModifiedTime(category.getModifiedTime());
                }
                if (category.getModifiedBy() != null) {
                    existingCategory.setModifiedBy(category.getModifiedBy());
                }
                if (category.getBackupCycle() != null) {
                    existingCategory.setBackupCycle(category.getBackupCycle());
                }
    ...
    public List<Category> searchCategories(@RequestParam String query) {
        ...
        return StreamSupport.stream(categorySearchRepository.search(query).spliterator(), false).collect(Collectors.toList());
    }
}
